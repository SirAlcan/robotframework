...
#${API_KEY_HEADER_PREFIX}   03ac2ca0-2815-41eb-894f-9d3a80c6c9da
@{PENDING_MARKS}
${DOC_COUNTER}             ${0}
...
Scenario 07 - Mixed Complex Flow
    ...
    Send 11.1 Debit    gross=20    expected_marks_count=3   # connects [M1,M2,M3]
    Send 8.6 Debit     gross=30        # M4
    Send 11.1 Debit    gross=30    expected_marks_count=1   # connects [M4]
    Send 8.6 Debit     gross=20        # M5
    Send 8.6 Credit    gross=20        # M6
    Send 11.4 Credit   gross=10    expected_marks_count=2   # connects [M5,M6]
...
Send 11.1 Debit
    ...
    ${marks_to_use}=      Pop Marks From Pool    close_all=${close_all_pending}
...
Send 11.4 Credit
    [Arguments]           ${gross}    ${expected_marks_count}=${NONE}
    ...                   ${vat_rate}=${DEFAULT_VAT_RATE}    ${close_all_pending}=${True}
    [Documentation]       Retail πιστωτική απόδειξη. Ίδιο schema με 11.1 αλλά InvoiceTypeCode=11.4.
    ${marks_to_use}=      Pop Marks From Pool    close_all=${close_all_pending}
    IF    $expected_marks_count is not None
        Length Should Be    ${marks_to_use}    ${expected_marks_count}
    END
